...
| 개인 맥락 (취업·사업·전략) | **이슈로 안 올린다** — 루트 `CLAUDE.md` 규칙 그대로 |
| 세션 간 인계·마일스톤 추적 | `BACKLOG.local.md` (git 미추적) |

⚠ **`BACKLOG.local.md` 와 진실원이 갈리는 문제가 열려 있다.** 루트 `CLAUDE.md` 는 그 파일을
*"세션 간 할 일 인계 + 마일스톤 추적의 진실원"* 으로 두고 있고, 이슈를 쓰기 시작하면
「다음에 뭘 하나」가 두 곳이 된다. **아직 안 정해졌다** — 정해지기 전까지는 위 표의 분담으로 쓴다.
